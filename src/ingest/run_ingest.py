...
def main():
    ...
    for y in years:
        ...
        checked_at = _utc_now_str()

        if last_ok is not None:
            last_ok_raw_path = str(last_ok.get("raw_path", ""))
            last_ok_abs = paths.root / last_ok_raw_path if last_ok_raw_path else None
            if (last_ok_abs is None) or (not last_ok_abs.exists()):
                logger.warning(f"[ingest] last OK missing on disk year={y} -> will restore by downloading")
                last_ok = None
        ...
        if is_historical and last_ok is not None:
            ...
            try:
                ...
                if not prev_abs.exists():
                    ...
                    last_updated_at = str(last_ok.get("downloaded_at_utc", "")) or checked_at
                    ...
                    append_manifest(
                        ...
                        ManifestRow(
                            ...
                            downloaded_at_utc=last_updated_at,
                    ...
                    try:
                        if prev_abs.resolve() != final_out.resolve():
                            prev_abs.unlink(missing_ok=True)
                    except Exception:
                        pass
                    continue
        ...
        if is_refresh:
            ...
            try:
                ...
                last_updated_at = str(last_ok.get("downloaded_at_utc", "")) if last_ok is not None else ""
