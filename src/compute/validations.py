...
def warn_missing_weeks(df: pd.DataFrame) -> list[str]:
    """
    Warn if there are gaps larger than 7 days between consecutive report_date values.

    Returns list of warning messages.
    """
    warnings: list[str] = []
    if "market_key" not in df.columns or "report_date" not in df.columns:
        return warnings

    df_sorted = df.sort_values(["market_key", "report_date"]).reset_index(drop=True)
    gap_days = 8
    for market_key, group in df_sorted.groupby("market_key"):
        dates = pd.to_datetime(group["report_date"], errors="coerce")
        deltas = dates.diff().dt.days
        gaps = deltas[deltas > gap_days]
        if not gaps.empty:
            warnings.append(
                f"Missing weeks WARN: {market_key} has {len(gaps)} gaps > {gap_days} days "
                f"(max gap {int(gaps.max())} days)"
            )
    return warnings


def warn_negative_open_interest(df: pd.DataFrame) -> list[str]:
    """
    Warn if open_interest_all has negative values.
    """
    warnings: list[str] = []
    if "open_interest_all" in df.columns:
        negative = (df["open_interest_all"] < 0).sum()
        if negative > 0:
            warnings.append(f"Open Interest WARN: {negative} rows with negative open_interest_all")
    return warnings


def warn_duplicate_keys(df: pd.DataFrame, keys: list[str]) -> list[str]:
    """Warn if duplicates exist on key columns."""
    warnings: list[str] = []
    dup_count = df.duplicated(subset=keys).sum()
    if dup_count > 0:
        warnings.append(f"Duplicate keys WARN: {dup_count} rows on keys: {', '.join(keys)}")
    return warnings


def warn_oi_missing_mid_history(df: pd.DataFrame) -> list[str]:
    """
    Warn if open_interest has NaN after the first non-NaN value per market_key.
    """
    warnings: list[str] = []
    if "market_key" not in df.columns or "report_date" not in df.columns:
        return warnings
    ...
    for market_key, group in df_sorted.groupby("market_key"):
        ...
        first_valid_idx = oi.first_valid_index()
        if first_valid_idx is None:
            continue
        # Any NaN after the first valid value counts as missing mid-history
        tail = oi.loc[first_valid_idx:]
        missing_after = tail.isna().sum()
        if missing_after > 0:
            warnings.append(
                f"Open Interest WARN: {market_key} has {missing_after} NaN values after initial data"
            )
    return warnings


def info_oi_chg_pct_threshold(df: pd.DataFrame, threshold: float) -> list[str]:
    """Info if abs(open_interest_chg_1w_pct) exceeds threshold."""
    infos: list[str] = []
    if "open_interest_chg_1w_pct" not in df.columns:
        return infos
    series = pd.to_numeric(df["open_interest_chg_1w_pct"], errors="coerce").abs()
    count = (series > threshold).sum()
    if count > 0:
        max_val = series.max()
        infos.append(
            f"Open Interest INFO: {count} rows with abs(open_interest_chg_1w_pct) > {threshold:.2f} "
            f"(max {max_val:.2f})"
        )
    return infos


def warn_oi_chg_pct_threshold(df: pd.DataFrame, threshold: float) -> list[str]:
    """Warn if abs(open_interest_chg_1w_pct) exceeds threshold."""
    warnings: list[str] = []
    if "open_interest_chg_1w_pct" not in df.columns:
        return warnings
    series = pd.to_numeric(df["open_interest_chg_1w_pct"], errors="coerce").abs()
    ...
    if count > 0:
        max_val = series.max()
        warnings.append(
            f"Open Interest WARN: {count} rows with abs(open_interest_chg_1w_pct) > {threshold:.2f} "
            f"(max {max_val:.2f})"
        )
    return warnings
...
def validate_max_min_all(df: pd.DataFrame) -> list[str]:
    """
    Validate ALL window when min == max.

    If min == max, pos_all should be 0.5 (or NaN if current value is NaN).
    """
    errors = []
    groups = ["nc", "comm"]
    sides = ["long", "short", "total"]
    ...
    for group in groups:
        for side in sides:
            ...
            pos_col = f"{group}_{side}_pos_all"

            if min_col not in df.columns or max_col not in df.columns or pos_col not in df.columns:
                continue
            ...
            pos_vals = df.loc[max_eq_min, pos_col].dropna()
            if not pos_vals.empty:
                bad = (np.abs(pos_vals - 0.5) > tol).sum()
                if bad > 0:
                    errors.append(
                        f"{pos_col}: {bad} rows where min==max but pos_all != 0.5 (tolerance {tol})"
                    )
...
def validate_max_min_5y(df: pd.DataFrame) -> list[str]:
    """
    Validate 5Y window when min == max.

    If min == max, pos_5y should be 0.5 (or NaN if current value is NaN).
    """
    errors = []
    groups = ["nc", "comm"]
    sides = ["long", "short", "total"]
    ...
    for group in groups:
        for side in sides:
            ...
            pos_col = f"{group}_{side}_pos_5y"
            ...
            if not pos_vals.empty:
                ...
                if bad > 0:
                    errors.append(
                        f"{pos_col}: {bad} rows where min==max but pos_5y != 0.5 (tolerance {tol})"
                    )
